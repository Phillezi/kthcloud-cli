...
func From(job any) *DeploymentJob {
	switch v := job.(type) {
...
func (job *DeploymentJob) Track(client *deploy.Client, ctx context.Context, deploymentName string, tickerInterval time.Duration, onCancel func()) error {
	if job.JobID == nil {
	...
	for {
		...
		case <-ctx.Done():
			...
			return nil
		case <-ticker.C:
			jobStatus, err := client.JobByID(*job.JobID)
			if err != nil {
