...
/*
func ParseComposeFile(filename string) (map[string]service.Service, error) {
	data, err := os.ReadFile(filename)
	if err != nil {
		return nil, fmt.Errorf("failed to read file: %w", err)
	}

	var composeFile composeFile.Compose
	...
	for name, serviceData := range composeFile.Services {
		...
		if err := parseService(serviceData, &service); err != nil {
			return nil, err
		}
...
func parseService(serviceData map[string]interface{}, service *service.Service) error {
