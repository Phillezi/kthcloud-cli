...
func (c *Command) build() error {
	if c.buildAll {
		...
		for n, s := range c.compose.Services {
			if s.Build != nil {
	...
	buildsReq, err := builder.GetBuildsRequired(c.client, *c.compose)
	if err != nil {
		logrus.Fatalln("Error getting builds required:", err)
	}
	for n, needsBuild := range buildsReq {
		if needsBuild {
			if err := builder.Build(c.client, c.ctx, n, c.compose.Services[n], c.nonInteractive); err != nil {
				logrus.Fatalln("Could not build service:", n, "Error:", err)
			}
			logrus.Debugln("build of", n, "is done!")
		}
